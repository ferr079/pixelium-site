...
async function liveCtfStats(): Promise<Record<string, string>> {
  const fb: Record<string, string> = { HTB_RANK: '594', HTB_RANK_NAME: 'Pro Hacker', HTB_FLAGS: '100', HTB_MACHINES: '49', ROOTME_SCORE: '1050' };
  try {
...
- Security doctrine: internal PKI (step-ca), SSO (Authentik), IPS (CrowdSec, 57 scenarios), SIEM (Wazuh), VPN mesh (Headscale), SSH hardened 63 hosts, YubiKey FIDO2
...
- Security: Authentik SSO, CrowdSec IPS (57 scenarios), Wazuh SIEM, Headscale VPN mesh, Infisical secrets manager + KeePassXC, SSH hardened 63 hosts, YubiKey FIDO2
- Observability: VictoriaMetrics, Grafana, Loki+Alloy, Beszel (51 agents), Uptime-Kuma, Patchmon
- Automation: 56 Ansible playbooks, 63 hosts, Semaphore (manual runs) + Dagu (scheduled DAGs)
