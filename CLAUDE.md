...
|---|---|---|
| Hack The Box | Ferr079 | Pro Hacker rank, #501, 53 system + 54 user owns, 107 flags (fallback ctf.astro + build-stats.ts + chat.ts + about.astro `DynNum` aligné sur le KV `/api/stats` — cf [[reference_ctf_htb_stats_freshness]], garder ce fallback à jour à la main). Lvl 63 « Master » · Season 11 **Holo** tier #35 (sur 12 660) · streak 12 sem. (screenshot `htb-dashboard.webp` MAJ 2026-08-09 = Pro Hacker/Lvl 63/Holo #35/12-week, 1200x619 ; `?v=20260809` ; **capture recadrée par Stéphane** sur la zone haute — le dashboard entier fait un ratio 1,37 = 876 px de haut à 1200 de large, trop encombrant, et ses graphes/badges deviennent illisibles à cette échelle). ⚠️ `/profile/activity` **supprimé par HTB 2026** (HTTP 400) → `flags = user_owns + system_owns`, ne PAS ré-ajouter d'appel activity. NB : `ranking` = rang **global** HTB (≠ `#373` = rang de saison du screenshot) ; le « HTB Rank » du dashboard (Professional → **Master** au Lvl 61) est une **2ᵉ échelle**, distincte du `rank` de l'API (`Pro Hacker`) — les deux coexistent dans la caption |
