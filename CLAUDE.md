...
|---|---|---|
| Hack The Box | Ferr079 | Pro Hacker rank, #633, 46 system + 49 user owns, 95 flags (fallback ctf.astro + build-stats.ts + chat.ts + about.astro `DynNum` aligné sur le KV `/api/stats` — cf [[reference_ctf_htb_stats_freshness]], garder ce fallback à jour à la main). Lvl 60 « Professional » · Season 11 Platinum tier #482 · streak 11 sem. (screenshot `htb-dashboard.webp` MAJ 2026-07-28 = Pro Hacker/Lvl 60/Platinum #482/11-week ; `?v=20260728`). ⚠️ `/profile/activity` **supprimé par HTB 2026** (HTTP 400) → `flags = user_owns + system_owns`, ne PAS ré-ajouter d'appel activity. NB : `ranking` = rang **global** HTB (≠ `#482` = rang de saison du screenshot) |
