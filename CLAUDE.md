...
|---|---|---|
| Hack The Box | Ferr079 | Pro Hacker rank, #819, 39 system + 42 user owns, 81 flags (fallback ctf.astro ; live au build via `/profile/basic`, mais le fetch CI tombe souvent en repli — cf [[reference_ctf_htb_stats_freshness]], garder ce fallback à jour à la main). Lvl 54 « Professional » · Season 11 Ruby tier #710 (screenshot `htb-dashboard.webp`, MAJ 2026-07-05 — ⚠️ screenshot périmé : montre encore Hacker/Lvl 52/#336, à rafraîchir). ⚠️ `/profile/activity` **supprimé par HTB 2026** (HTTP 400) → `flags = user_owns + system_owns`, ne PAS ré-ajouter d'appel activity. NB : `ranking` = rang **global** HTB (≠ `#336` = rang de saison du screenshot) |
