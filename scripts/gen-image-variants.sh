...
# Regenerate the width map so assetSrcset() emits an honest `Nw` for the original
# candidate (widths vary 1200..2944px). Local file — refreshed unless --dry-run.
if [ "$DRY_RUN" = "--dry-run" ]; then
  echo "DRY: régénérerait src/image-dims.ts (largeurs réelles des originaux)"
else
  MAP="src/image-dims.ts"
  {
    echo "// AUTO-GENERATED by scripts/gen-image-variants.sh — do not edit by hand."
    echo "// Real pixel widths of image originals, so assetSrcset() declares an honest"
    echo "// \`Nw\` for the original candidate (widths vary 1200..2944px). Regenerate after"
    echo "// adding/replacing images:  ./scripts/gen-image-variants.sh"
    echo "export const IMAGE_WIDTHS: Record<string, number> = {"
    find public/images -name '*.webp' ! -name '*-480.webp' ! -name '*-800.webp' | sort | while read -r f; do
      rel="${f#public/}"
      echo "  \"/${rel}\": $(magick identify -format '%w' "$f"),"
    done
    echo "};"
  } > "$MAP"
  echo "wrote $MAP"
